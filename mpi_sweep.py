...
# Ensure single-threaded BLAS/MKL per rank
...
# MPI setup\comm = MPI.COMM_WORLD
rank = comm.Get_rank()
size = comm.Get_size()

# Unique cache per rank
...
# Sweep configuration: 50 gamma points divided into `size` chunks
...
def main():
    ...
    # Initialize solver and JIT-compile
    solver = fem.SubCubes(
        domain=domain,
    ...
    local_results = []
    # Outer sweep: each "thing" handles its own sub-range of gamma
    for gamma in my_gammas:
        # Nelder–Mead optimization of gamma_a, gamma_R
    ...
    # Gather and post-process on rank 0
