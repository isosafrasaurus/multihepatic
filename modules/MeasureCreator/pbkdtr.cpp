...
private:
    ...
    void insertRec(KDNode *node, const Vec3 &p, float radius, int depth) {
        int axis = depth % 3;
        ...
        if(pVal < nodeVal) {
            if(node->left == nullptr) {
                ...
                insertRec(node->left, p, radius, depth + 1);
            }
        } else {
            if(node->right == nullptr) {
                node->right = new KDNode(p, radius, node);
            } else {
                insertRec(node->right, p, radius, depth + 1);
            }
        }
    }
    float queryRec(KDNode *node, const Vec3 &p) {
        ...
        float ret = queryRec(node->left, p);
...
PYBIND11_MODULE(kd_tree, m) {
    m.doc() = "pybind11 bindings for KDTree";

    py::class_<KDTree>(m, "KDTree")
        .def(py::init<>())
        .def("insert", [](KDTree &self, py::array_t<float> p, float radius) {
            if (p.ndim() != 1 || p.shape(0) != 3)
                throw std::runtime_error("Input must be a 1D array of size 3");
            auto buf = p.unchecked<1>();
            Vec3 point(buf(0), buf(1), buf(2));
            self.insert(point, radius);
        })
        .def("query", [](KDTree &self, py::array_t<float> p) {
            if (p.ndim() != 1 || p.shape(0) != 3)
                throw std::runtime_error("Input must be a 1D array of size 3");
            auto buf = p.unchecked<1>();
            Vec3 point(buf(0), buf(1), buf(2));
