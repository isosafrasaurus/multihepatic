...
def run_sweep(sweep_name, sweep_values, target, solver, x_default=None, maxiter_o=50, maxiter_c=50, directory=None):
    """
    ...
    Args:
        ...
        target (float): The target net flow value.
        solver (fem.SubCubes): The SubCubes solver object to use.
    Returns:
        pandas.DataFrame: A DataFrame containing the results of the sweep.
    """
    ...
    if directory is not None:
        ...
        df = sweep_variable(
            ...
            solver,
            maxiter_o,
            os.path.join(real_directory, "data.csv")
        )
        x = df[sweep_name]
        ...
        plt.tight_layout()
        lower_path = os.path.join(real_directory, "lower_cube_plot.pdf")
        ...
        plt.grid(True, which='both', linestyle='--', linewidth=0.1)
        plt.tight_layout()
        upper_path = os.path.join(real_directory, "upper_cube_plot.pdf")
