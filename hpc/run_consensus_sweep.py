...
def run_sweep_sublist(*, sweep_name, sweep_values, part_idx, out_dir,
                      ...
                      x_default=None) -> Path:
    """
    Executes *only* the values in `sweep_values` and writes
    {out_dir}/part_{part_idx:03d}.csv
    """
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    # The heavy lifting – reuse original routine but turn off plotting
    df = sweep_variable(
        ...
        x_default   = x_default,
        target      = target,
        solver      = None,      # let sweep_variable build its own solver
        maxiter     = maxiter_o,
        save_path   = None
    )
