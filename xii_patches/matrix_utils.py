...
def petsc_serial_matrix(test_space, trial_space, nnz=None):
    ...
    if is_number(test_space) and is_number(trial_space):
        # In the scalar-size case there is no way to infer ownership in parallel.
        # Keep this strictly serial; refuse MPI to avoid corrupt layouts.
        comm = MPI.comm_world
        if comm.size != 1:
    ...
    # With function space this can be extracted
        ...
        # IMPORTANT: LGMaps must describe OWNED rows/cols only
