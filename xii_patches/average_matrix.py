...
def memoize_average(average_mat):
    ...
    def cached_average_mat(V, TV, reduced_mesh, data):
        # Include communicator size to prevent accidental reuse across
...
def average_matrix(V, TV, shape):
    ...
    # MPI-safe: tabulate_dof_coordinates() returns coordinates for dofs
    # local to this rank (owned + possibly ghosts). Never reshape with the
    # GLOBAL size (TV.dim()). Use geometric dimension so NumPy infers the
    # correct LOCAL row count.
    gdim = TV.mesh().geometry().dim()
    TV_coordinates = TV.tabulate_dof_coordinates().reshape((-1, gdim))
    line_mesh = TV.mesh()
    
    # Full-space dofmaps and local->global maps
    ...
    # For vector-valued TV we iterate scalar dofs via component-0 subspace,
    # then offset rows by component 'shift'.
        ...
        for line_cell in tqdm.tqdm(cells(line_mesh), desc=f'Averaging over {line_mesh.num_cells()} cells',
            ...
            for scalar_row, avg_point in zip(scalar_dofs, scalar_dofs_x):
                ...
                for shift in range(value_size):
                    ...
                    # Insert only owned rows from this rank to avoid duplication
                    if row_local >= nrow_owned:
                        continue
                    # Cast to PETSc.IntType (portable between 32/64-bit PETSc)
                    ...
                    # Global insertion; ADD to accumulate across ranks
...
def scalar_average_matrix(V, TV, shape):
    ...
    gdim = TV.mesh().geometry().dim()
    TV_coordinates = TV.tabulate_dof_coordinates().reshape((-1, gdim))  # LOCAL
    line_mesh = TV.mesh()
    ...
    # Global insertion with accumulation; avoid building a global CSR per rank.
