...
def apply_bc(A, b, bcs, diag_val=1., symmetric=True, return_apply_b=False):
    ...
    # MPI-aware fast path: avoid collapsing to a serial monolithic matrix.
    # Use cbc.block's parallel-safe boundary-condition machinery instead.
    comm = MPI.comm_world
    if comm.size > 1:
        # Ensure A is a block_mat of PETScMatrix (concrete matrices), not compositions.
        A = convert(A, algorithm=None)

        # Attach proper row LGMaps so MatZeroRowsLocal can work.
        # For each block-row i that actually has DirichletBCs, derive the row LGMap
        # from that row's function space, and set it on A[i, j] for all j.
        for i, row_bcs in enumerate(bcs):
            # Skip rows without BCs
            if not row_bcs:
                continue
            # Unwrap if it's e.g. a dict or a plain DirichletBC
            if isinstance(row_bcs, dict):
                row_bcs = sum(row_bcs.values(), [])
            if isinstance(row_bcs, DirichletBC):
                row_bcs = [row_bcs]
            # Find the function space that BCs target (all must agree)
            row_V = None
            for bc in row_bcs:
                if isinstance(bc, DirichletBC):
            ...
            # Build row LGMap from the DOF map (OWNED dofs)
            dm = row_V.dofmap()
            ...
            # Attach to every block in row i (use a safe column LGMap if needed)
            for j in range(len(A[i])):
                Aij = A[i][j]
                # PETSc Mat
                mat = as_backend_type(Aij).mat()
                # Try to reuse existing column LGMap; if missing, synthesize one
                try:
                    col_lgmap = mat.getLGMap()[1]
                except Exception:
                    col_lgmap = None
                if col_lgmap is None:
                    # Synthesized from column ownership range (contiguous global cols)
        ...
        op.apply(A)
        if b is not None:
            # For the RHS, use the dedicated block_rhs_bc operator
