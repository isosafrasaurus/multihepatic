...
def convert(bmat, algorithm='numpy'):
    ...
    if isinstance(bmat, block_vec):
        # MPI-safe: build a parallel Vec by concatenating component Vecs
        comm = MPI.comm_world
        # Fast serial path (kept for backwards-compatibility)
        if comm.size == 1:
        ...
        # Parallel path
        comp = [as_petsc(v) for v in bmat]            # PETSc.Vec for each block
        sizes = [int(v.getSize()) for v in comp]      # global sizes per block
        N = int(sum(sizes))
        # Local size is sum of owned entries from all blocks on this rank
        nloc = int(sum((v.getOwnershipRange()[1] - v.getOwnershipRange()[0]) for v in comp))
        out = PETSc.Vec().create(comm=COMM)
        out.setSizes((nloc, N))                       # explicit (local, global) sizes
        out.setUp()
        # Fill owned chunk of each component into the concatenated vector
        offset = 0
        for v, sz in zip(comp, sizes):
            r0, r1 = v.getOwnershipRange()                         # owned global range in this component
            comp_idx = np.arange(r0, r1, dtype=PETSc.IntType)
            vals = v.getValues(comp_idx)                           # numpy array of values
            tgt_idx = comp_idx + offset                            # indices in concatenated vector
    ...
    if isinstance(bmat, block_mat):
        ...
        # Opt out of monolithic collapse: return as block_mat
        if not algorithm:
            set_lg_map(bmat)
            return bmat
        ...
        comm = MPI.comm_world
        if comm.size > 1:
            # Build a MatNest from concrete submatrices and convert to AIJ.
            ...
            Nest.assemble()
            Aij = Nest.convert('aij')   # parallel AIJ with correct mapping
...
def collapse_tr(bmat):
    ...
    if is_petsc_mat(A):
        ...
        C_ = PETSc.Mat()
        A_.transpose(C_)
        # Propagate LGMaps (swap on transpose)
        try:
            rA, cA = A_.getLGMap()
...
def collapse_mul(bmat):
    ...
    if len(B) == 1:
        ...
        if is_petsc_mat(A) and is_petsc_mat(B):
            ...
            C_ = PETSc.Mat()
            A_.matMult(B_, C_)
            # Propagate LGMaps: rows from A, cols from B
            try:
                rA, _ = A_.getLGMap()
            except Exception:
                rA = None
            ...
            try:
                if rA is not None or cB is not None:
                    # if one side missing, PETSc still accepts None for the other
                    C_.setLGMap(rA, cB)
            except Exception:
                pass
            return PETScMatrix(C_)
...
def set_lg_map(mat):
    ...
    # Only set a synthetic identity LGMap in serial. In parallel the matrix
    # already carries proper LGMaps or is assembled with them.
    try:
