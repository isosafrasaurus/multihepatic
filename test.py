...
def _build_1d_mesh_serial_from_graph(G: FenicsGraph, nref: int):
    ...
    editor = MeshEditor()
    editor.open(mesh, "interval", 1, vertex_coords.shape[1])  # topological dim 1, geom dim 3
...
def main() -> float:
    ...
    # 3D box via explicit bounds; Robin sink on x=0 plane
    ...
    with Domain1D(G, Lambda_num_nodes_exp=TEST_NUM_NODES_EXP, inlet_nodes=[0]) as Lambda, \
        ...
        # Collective solve on all ranks (Design A primary entry point)
        sol = sim.run(params)   # (alias: sim.solve(params) still works)
        ...
        # Drop references (or simply: sol.close())
