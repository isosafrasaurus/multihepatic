...
def build_assembled_forms(
    ...
    if Omega_sink_subdomain is not None:
        # If we were given a MeshFunction (e.g. from sink_markers_from_surface_vtk),
        # just copy its values onto boundary_Omega.
        if isinstance(Omega_sink_subdomain, MeshFunction) or hasattr(
            ...
            # SubDomain-like object: use .mark(...)
    ...
    if inlet_nodes:
        for node_id in inlet_nodes:
            BoundaryPoint(
                G.nodes[node_id]["pos"],
                tolerance=DOLFIN_EPS,
            ).mark(boundary_Lambda, 1)
    ...
    a01 = -consts.gamma * u1 * v3_avg * D_peri * dxLambda - (
            consts.gamma_a / consts.mu
    ) * u1 * v3_avg * D_area * dsLambdaRobin
    ...
    L0 = (
            ...
            + (consts.gamma_a * consts.P_cvp / consts.mu)
            * v3_avg
            * D_area
            * dsLambdaRobin
    )
