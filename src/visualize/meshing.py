...
def build_fenicsgraph_from_json(directory):
    ...
    for idx, file_name in enumerate(json_files):
        ...
        # Check if the first control point of the current file is a branch point from previous files.
        v1 = 0
        ...
        # Add the first edge in this centerline segment
        v2 = ind - idx + 1
        ...
        G.add_edge(v1, v2)
        # Assign edge radius as the average of the node radii.
        G.edges[v1, v2]["radius"] = (G.nodes[v1]["radius"] + G.nodes[v2]["radius"]) / 2

        # Build the rest of the centerline segment
